"""
Binary ViT Evaluation & Visualization Module

Standalone script for generating training plots and classification metrics
for the binary (damaged vs. not-damaged) Vision Transformer model.
...
Usage:
    python src/evaluation/metrics_binary.py
"""
import csv
import json
import sys
...
def plot_training_curves(history_path: str | Path, output_dir: str | Path):
    ...
    axes[0].set_title('Training & Validation Loss (Binary)', fontsize=14, fontweight='bold')
    ...
    axes[1].set_title('Training & Validation Accuracy (Binary)', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plot_path = Path(output_dir) / "vit_binary_training_curves.png"
...
def plot_confusion_matrix(cm: np.ndarray, output_dir: str | Path):
    ...
    ax.set_title('Confusion Matrix — Binary ViT', fontsize=14, fontweight='bold')
    plt.xticks(rotation=0)
    plt.yticks(rotation=0)

    plt.tight_layout()
    cm_path = Path(output_dir) / "vit_binary_confusion_matrix.png"
...
def save_metrics_csv(all_labels: np.ndarray, all_preds: np.ndarray,
    ...
    metrics_path = Path(output_dir) / "vit_binary_metrics.csv"
    with open(metrics_path, 'w', newline='') as f:
        writer = csv.writer(f)

        writer.writerow(["=== Binary ViT Classification Metrics ==="])
        writer.writerow([])
...
def evaluate_model(model_path: str | Path, data_dir: str | Path, output_dir: str | Path):
    """
    Loads the trained binary ViT model and evaluates it against the full dataset.
    """
    ...
    print("[*] Loading best Binary ViT model...")
...
def run_full_evaluation(split="test"):
    """
    Orchestrates the binary evaluation pipeline by plotting training curves,
    ...
    model_path = PROJECT_ROOT / "results" / "models" / "best_vit_binary.pth"
    history_path = results_dir / "training_history_binary.json"

    results_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print(f"  Binary ViT — Full Evaluation Pipeline ({split} split)")
    print("=" * 60)
    ...
    if not model_path.exists():
        ...
        print("         Run train_vit_binary.py first.")
        return
