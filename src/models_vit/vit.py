...
class PatchEmbedding(nn.Module):
    ...
    """
    def __init__(self, in_channels=6, patch_size=8, embed_dim=256, img_size=64):
        super().__init__()
    ...
    def forward(self, x):
        # x shape: [Batch, Channels, Height, Width] -> [B, 6, 64, 64]
        x = self.proj(x) 
...
class VisionTransformer(nn.Module):
    ...
    Expected Input Shape: [Batch, 6, 64, 64]
    Outputs: [Batch, 4]
    
    Why 64x64 Image Size?
    Building crops from YOLO are generally localized and small. 64x64 ensures enough
    resolution to distinguish structural edges and debris while keeping the token sequence
    length small (8x8 patches -> 64 tokens + 1 CLS token = 65 sequence length).
    Since Self-Attention scales quadratically with sequence length, this ensures lightning-fast
...
if __name__ == "__main__":
    print("Testing Vision Transformer Initialization and Tensor Shapes...\n")
    
    # 1. Instantiate the Model
    # 6 Channels, 64x64 Image, 8x8 Patches, 4 Output Classes
    model = VisionTransformer(img_size=64, patch_size=8, in_channels=6, num_classes=4)
    
    # 2. Create Dummy Data
    # Batch Size of 2, 6 Channels (Pre+Post), 64x64 Resolution
    dummy_input = torch.randn(2, 6, 64, 64)
    ...
    print(f"Output Shape: {output_logits.shape} -> [Batch, NumClasses]")
    
    assert output_logits.shape == (2, 4), "ERROR: Output shape does not match expected (2, 4)"
    print("\nSUCCESS: Vision Transformer architecture built and tensor flow verified!")
