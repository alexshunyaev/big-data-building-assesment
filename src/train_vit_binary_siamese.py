"""
Siamese Binary ViT Training Script

Trains the SiameseChangeViT model for damaged-vs-not-damaged classification.
Uses BCEWithLogitsLoss (with optional pos_weight for class imbalance) and
the same training recipe as the previous variants.
"""
...
def train_siamese_binary_model():
    """
    Main training function for the Siamese binary damage detection model.
    """
    # ---------------------------------------------------------------
    # 1. Configuration
    # ---------------------------------------------------------------
    ...
    print(f"[*] Training SIAMESE BINARY ViT on device: {device}")
    ...
    # ---------------------------------------------------------------
    # Note: in_channels=3 because the model processes pre and post separately
    ...
    pos_weight = class_sample_counts[0] / class_sample_counts[1]
    ...
    # 4. Training Loop
    # ---------------------------------------------------------------
    print(f"\n[*] Starting Siamese Binary Training Loop ({epochs} epochs, patience={patience})...")
    ...
    for epoch in range(epochs):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            epochs_no_improve = 0
            torch.save(model.state_dict(), SAVE_DIR / "best_vit_binary_siamese.pth")
            print(f"      [!] New best SIAMESE model saved (Val Loss={epoch_val_loss:.4f})")
        else:
            epochs_no_improve += 1
    ...
    print(f"\n[*] Siamese Binary Training Complete. Best Validation Loss: {best_val_loss:.4f}")


if __name__ == "__main__":
    train_siamese_binary_model()
